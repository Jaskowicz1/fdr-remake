...
int main(int argc, char *argv[]) {
    ...
    bot.on_message_create([&rcon_client, msg_channel](const dpp::message_create_t& event) -> dpp::task<void> {
    ...
    bot.on_slashcommand([&rcon_client](const dpp::slashcommand_t& event) {
        if (event.command.get_command_name() == "evolution") {
            ...
            rcon_client.send_data("/command ", 3, data_type::SERVERDATA_EXECCOMMAND, [event](const std::string& data) {
                event.reply(dpp::message(data).set_flags(dpp::m_ephemeral));
            });
        }
    });
    ...
    bot.on_ready([&bot, &rcon_client](const dpp::ready_t& event) {
        ...
        if (dpp::run_once<struct register_bot_commands>()) {
            ...
            bot.global_bulk_command_create({ evolution_command, time_command, version_command, players_command, seed_command });
        }
